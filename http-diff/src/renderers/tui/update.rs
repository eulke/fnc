...
impl Default for Effect {
    fn default() -> Self { Effect::None }
}

pub fn update(app: &mut TuiApp, msg: Msg) -> Effect {
    match msg {
        Msg::Quit => Effect::Quit,
        Msg::ToggleHelp => { app.toggle_help(); Effect::None }
        Msg::ToggleExpanded(panel) => { app.toggle_panel_expansion(panel); Effect::None }
        Msg::FocusNextPane => { app.next_dashboard_panel(); Effect::None }
        Msg::FocusPrevPane => { app.previous_dashboard_panel(); Effect::None }
        Msg::ToggleDiffStyle => { app.toggle_diff_style(); Effect::None }
        Msg::ToggleHeaders => { app.toggle_headers(); Effect::None }
        Msg::ToggleErrors => { app.toggle_errors(); Effect::None }
        ...
        Msg::StartExecution => {
            if app.selected_environments.is_empty() || app.selected_routes.is_empty() {
                app.show_feedback("Select at least one environment and route", super::app::FeedbackType::Warning);
                Effect::None
            } else {
                app.start_execution();
                Effect::StartExec {
        ...
        Msg::Exec(em) => {
            match em {
                ExecMsg::Progress { completed, total, op } => {
...
fn handle_results(app: &mut TuiApp, msg: ResultsMsg) -> Effect {
    match msg {
        ...
        ResultsMsg::PageUp => { app.selected_index = app.selected_index.saturating_sub(10); app.sync_results_table_state(); }
        ResultsMsg::PageDown => {
        ...
        ResultsMsg::Home => { app.selected_index = 0; app.sync_results_table_state(); }
        ResultsMsg::End => { let max = app.filtered_results().len().saturating_sub(1); app.selected_index = max; app.sync_results_table_state(); }
