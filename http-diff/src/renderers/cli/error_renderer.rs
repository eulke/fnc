...
impl ErrorRenderer {
    ...
    fn format_error_group(&self, group: &ErrorGroup) -> String {
        ...
        for route_error in &group.affected_routes {
            ...
            let status_display =
                if route_error.has_consistent_status && !route_error.status_codes.is_empty() {
            ...
            let tree_symbol = if self.use_unicode_symbols {
                "└─"
            } else {
                "+-"
            };
...
mod tests {
    ...
    fn create_test_error_group() -> ErrorGroup {
        ErrorGroup {
            ...
            affected_routes: vec![RouteError {
    ...
    fn test_severity_icons() {
        ...
        assert_eq!(
            renderer_with_emojis.get_severity_icon(&ErrorSeverity::Critical),
            "🔥"
        );
        assert_eq!(
            renderer_without_emojis.get_severity_icon(&ErrorSeverity::Critical),
            "[CRITICAL]"
        );

        assert_eq!(
            renderer_with_emojis.get_severity_icon(&ErrorSeverity::Client),
            "⚠️"
        );
        assert_eq!(
            renderer_without_emojis.get_severity_icon(&ErrorSeverity::Client),
            "[CLIENT]"
        );
    }
}
